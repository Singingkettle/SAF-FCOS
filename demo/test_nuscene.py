...
def main():
    ...
    parser.add_argument(
        "--config-file",
        default="configs/fcos_nuscenes/fcos_imprv_R_50_FPN_1x.yaml",
        metavar="FILE",
    ...
    for im_name in tqdm(demo_im_names):
        ...
        for i, label in enumerate(labels):
            if label in VEHICLES:
                box = boxes[i].tolist()
                valid_bboxs.append(box)

        im_file_prefix = im_name.split('.')[0]

        with open(os.path.join('/home/citybuster/nuscenes_fcos', im_file_prefix + '.txt'), 'w') as f:
            for data_info in valid_bboxs:
                f.write('{}, {:.3f}, {:.3f}, {:.3f}, {:.3f}\n'.format(0, data_info[0], data_info[1],
